...
create_role() {
    ...
    # Create trust policy
    cat > /tmp/${env_name}-trust-policy.json <<EOF
{
  "Version": "2012-10-17",
  "Statement": [
    {
...
EOF
    ...
    aws iam create-open-id-connect-provider \
        ...
        --region eu-central-1 2>/dev/null || echo "OIDC provider already exists"

    # Create role
    aws iam create-role \
        --role-name "$role_name" \
        --assume-role-policy-document file:///tmp/${env_name}-trust-policy.json \
        --description "GitHub Actions role for TByte ${env_name} environment" \
        --region eu-central-1 2>/dev/null || echo "Role may already exist"
    ...
    # Clean up
    rm -f /tmp/${env_name}-trust-policy.json
...
echo ""
echo "📋 Update GitHub Secrets with these ARNs:"
